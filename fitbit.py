...
def average_sleep(df_sleep, start_date=None, end_date=None):
    df_sleep = df_sleep.copy()
    if not pd.api.types.is_datetime64_any_dtype(df_sleep['date']):
    ...
    if start_date is not None:
        ...
        df_sleep = df_sleep[df_sleep['date'] >= start_date]
    
    if end_date is not None:
        if not isinstance(end_date, pd.Timestamp):
            end_date = pd.to_datetime(end_date)
        df_sleep = df_sleep[df_sleep['date'] <= end_date]
    
    # Assign time blocks
    df_sleep['TimeBlock'] = df_sleep['date'].dt.hour.apply(assignTimeBlock)
    
    # Group by time block and sum sleep values
    ...
    # Update title to include date range if provided
    title = 'Total Sleep Minutes per 4-Hour Block'
